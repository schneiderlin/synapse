...
(defn ws-event-handler
  ...
  [_system _event-msg]
  #?(:clj (api/ws-event-handler _system _event-msg)
...
(defn ws-event-handler-frontend
  ...
  [_system _event-msg]
  #?(:cljs
     (let [{:keys [id ?data]} _event-msg]
       (case id
         :xiangqi/game-state-update
         (let [store (:replicant/store _system)]
...
(defn fen->state
  ...
  [_fen-string]
  #?(:clj ((requiring-resolve 'com.zihao.xiangqi.fen/fen->state) _fen-string)))

(defn move-str->coords
  "Convert a move string (e.g. 'e0e1') to coordinate pairs [[from-row from-col] [to-row to-col]]"
  [_move-str]
  #?(:clj ((requiring-resolve 'com.zihao.xiangqi.fen/move-str->coords) _move-str)))

(defn coords->move-str
  "Convert coordinate pairs [[from-row from-col] [to-row to-col]] to a move string (e.g. 'e0e1')"
  [_coords]
  #?(:clj ((requiring-resolve 'com.zihao.xiangqi.fen/coords->move-str) _coords)))

(defn state->fen
  "Convert a xiangqi state map to a FEN string"
  [_state]
  #?(:clj ((requiring-resolve 'com.zihao.xiangqi.fen/state->fen) _state)))

(defn chessboard
  "Render a xiangqi chessboard from state"
  [_state]
  #?(:clj ((requiring-resolve 'com.zihao.xiangqi.render/chessboard) _state)))
