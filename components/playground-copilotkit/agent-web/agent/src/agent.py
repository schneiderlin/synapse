...
# Agent
# =====
provider = AnthropicProvider(api_key=os.getenv('ZAI_API_KEY'), base_url='https://api.z.ai/api/anthropic')

agent = Agent(
  model = AnthropicModel(
    'GLM-4.6',
    provider=provider,
    # Ensure streaming is enabled (default should be True, but explicitly set it)
  ),
